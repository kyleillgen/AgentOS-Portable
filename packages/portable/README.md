# AgentOS portable package 0.2.1 — early preview
...
- **Manual:** download [agentos-manual.zip](https://github.com/kyleillgen/AgentOS-Portable/releases/download/v0.2.1/agentos-manual-0.2.1.zip), extract into a new folder, and open START-HERE.md.
- **Agent-assisted:** download [agentos-agent-setup.zip](https://github.com/kyleillgen/AgentOS-Portable/releases/download/v0.2.1/agentos-agent-setup-0.2.1.zip), upload it to a locally connected agent, and ask it to read AGENT-SETUP.md. It inspects the laptop, guides logins, configures and tests the same dispatcher.
...
The destination must be new and its parent must exist. This command does not configure logins or start the dispatcher. The agent bundle's Install-AgentOS.ps1 copies the same template and runs setup.ps1 with the supplied executable and ledger paths.
...
See [ARCHITECTURE.md](ARCHITECTURE.md) for interoperability and ownership limits. Installation does not change an existing AgentOS environment, schedule, or account configuration.
