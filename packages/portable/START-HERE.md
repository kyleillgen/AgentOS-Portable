# Start here — AgentOS Portable 0.2.1
...
Download both options from the [0.2.1 release](https://github.com/kyleillgen/AgentOS-Portable/releases/tag/v0.2.1). Check SHA256SUMS.txt from the same release with PowerShell's Get-FileHash if you want to verify the downloaded bytes.
...
The [first-run checklist](template/FIRST-RUN.md) and [full CLI setup guide](template/runners/README.md) are under template/. The agent bundle's Install-AgentOS.ps1 copies that folder into a new destination and uses the same setup.ps1 as the manual path.
